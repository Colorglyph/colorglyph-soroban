...
# # glyph_mint
# glyph_hash=$(soroban contract invoke --id $contract_id --source $user1_sk --network Futurenet -- glyph_mint --minter $user1_pk --colors '{"'$user1_pk'": {"0": [3, 2], "16777215": [1, 0]}}' --width 2 | jq '.Hash[]' | tr -d '"')
# echo 'glyph minted' $glyph_hash
# # glyph_hash="74bf48c8c20e4f6fa3a94d548400de5832ee8c5daef3fc0c7eefc909b0142f20"

# # glyph_get
# soroban contract invoke --id $contract_id --source Default --network Futurenet -- glyph_get --hash_id '{"Hash": ["'$glyph_hash'"]}'

# # glyph_transfer
# soroban contract invoke --id $contract_id --source $user1_sk --network Futurenet --fee 1000000000 -- glyph_transfer --to $user2_pk --from $user1_pk --hash_id '{"Hash": ["'$glyph_hash'"]}'

# # offer_post
# soroban contract invoke --id $contract_id --source $user2_sk --network Futurenet -- offer_post --seller $user2_pk --sell '{"Glyph": ["'$glyph_hash'"]}' --buy '{"Asset": ["'$token_address'", "100"]}'
# echo 'offer posted'
...
# # offer_delete
# soroban contract invoke --id $contract_id --source $user2_sk --network Futurenet -- offer_delete --seller $user2_pk --sell '{"Glyph": ["'$glyph_hash'"]}' --buy '{"Asset": ["'$token_address'", "100"]}'
# echo 'offer deleted'

# # glyph_scrape
# soroban contract invoke --id $contract_id --source $user2_sk --network Futurenet -- glyph_scrape --owner $user2_pk --hash_id '{"Hash": ["'$glyph_hash'"]}'
# echo 'glyph scraped'